...
function algoArea() {
  ...
  if (a > 0 && b > 0) {
    ...
    document.getElementById("resultArea").textContent =
      "Área do Retângulo: " + area.toFixed(2) + " m²";
  } else {
...
function algoFat() {
  ...
  if (number < 0) {
    document.getElementById("resultFat").textContent =
...
function algoFib() {
  ...
  if (n <= 0) {
    document.getElementById("resultFib").textContent =
      "Por favor, insira um número positivo.";
    return;
  } else if (n >= 15) {
    document.getElementById("resultFib").innerHTML =
      "<p>O número é maior que desejado <br>Coloque um número entre 1 e 14!</p>";
  } else {
    let fibonacci = [0, 1];
...
function algoImposto() {
  ...
  const percent1 = 75 / 1000;
  const percent2 = 15 / 100;
  const percent3 = 225 / 1000;
  const percent4 = 275 / 1000;

  let deducao;

  if (n >= 0.0 && n <= 2428.8) {
    document.getElementById("resultImp").textContent =
      "Isento de imposto de renda";
  } else if (n > 2428.8 && n <= 5000.0) {
    mensagem = "O imposto foi zerado! Você pagaria a dedução mensal de R$ ";
    if (n > 2428.8 && n <= 2826.65) {
      deducao = n * percent1;
      document.getElementById("resultImp").textContent =
        mensagem + deducao.toFixed(2);
  ...
  } else if (n > 5000.0 && n <= 7350.0) {
    deducao = n * percent4;
    let reducao = 0.133145 * n;
    let valor = deducao - reducao;
    document.getElementById("resultImp").textContent =
      "O dedução do imposto mensal junto com a redução vai ser de R$" +
      valor.toFixed(2);
  } else {
    deducao = n * percent4;
    document.getElementById("resultImp").textContent =
      "O dedução do imposto mensal vai ser de R$" + deducao.toFixed(2);
  }
}
...
function algoMaior() {
  ...
  document.getElementById("resultMaior").textContent =
...
function algoMedia() {
  ...
  document.getElementById("resultMedia").textContent =
    `A media é ${med.toFixed(2)}.`;
}
...
function algoPrimo() {
  ...
  if (n <= 1) {
    document.getElementById("resultPrimo").textContent =
...
function algoOrde() {
  ...
  document.getElementById("resultOrde1").textContent =
    `Números em ordem crescente: ${descending[2]}, ${descending[1]}, ${descending[0]}.`;
}
